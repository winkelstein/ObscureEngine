...
{
  Config config = this->parseConfig(path_to_config);
  std::shared_ptr<Model> model = this->readFromFile(config.model);

  return std::make_pair(config.name, model);
}

ObscureEngine::Importer::ModelImporter::Config ObscureEngine::Importer::ModelImporter::parseConfig(std::filesystem::path path_to_config)
{
...
std::vector<uint32_t> ObscureEngine::Importer::ModelImporter::loadPalette(FILE *file, int totalSize)
{
...
// TODO: done
std::shared_ptr<ObscureEngine::Model> ObscureEngine::Importer::ModelImporter::parseModel(std::vector<VoxPos> positions, std::vector<VoxSize> sizes, std::vector<uint32_t> palette)
{
  ...
  for (uint32_t i = 0; i < positions.size(); i++)
  {
    ...
    voxels.push_back(std::shared_ptr<Voxel>(new Voxel(glm::vec4(r, g, b, a))));
...
std::shared_ptr<ObscureEngine::Model> ObscureEngine::Importer::ModelImporter::readFromFile(std::filesystem::path path_to_config)
{
  ...
  std::vector<uint32_t> palette = this->loadPalette(file, totalSize);
  ...
  std::shared_ptr<ObscureEngine::Model> model = this->parseModel(positions, sizes, palette);
  return model;
}